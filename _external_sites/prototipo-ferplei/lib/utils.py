...
def set_view_cookie_and_return_view(cls, force=""):
    """
    set language returning a dict and set cookie
    """
    if force != "":
        view_cookie = force
    elif cls.request.get("view") == "":
        # ask for cookie
        view_cookie = read_cookie(cls, "view")
        if not view_cookie:
            view_cookie = "mashup"
    else:
        # set cookie to param 'view' value
        view_cookie = cls.request.get("view")
        # Two weeks for view cookie
    write_cookie(cls, "view", str(view_cookie), "/", 1209600)
